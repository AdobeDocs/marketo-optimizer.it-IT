---
user-guide-title: Documentazione di Marketo Optimizer
user-guide-description: Scopri Adobe Marketo Optimizer e come utilizzarlo per eseguire attività coordinate di marketing e vendita con contenuti personalizzati e basati sull’intelligenza artificiale per lead e account B2B.
source-git-commit: 177e7c3d0806febd730104b19787ba3cbea2914a
workflow-type: tm+mt
source-wordcount: '252'
ht-degree: 42%
---

# Guida utente di Marketo Optimizer {#user}

+ [Documentazione di Adobe Marketo Optimizer](guide-overview.md)
+ [Architettura dei dati](data-architecture.md)
+ Introduzione {#start}
  + [Elenco di controllo dell&#39;installazione](./start/setup-check-list.md)
  + [Gestione degli utenti](./start/user-management.md)
  + [Recapitabilità delle e-mail](./start/email-deliverability.md)
+ [Pagina Home](./home-insights/home-page.md)
+ Funzionalità del collaboratore {#coworker}
  + [Interfaccia chat](./agents/chat-interface.md)
  + [Competenza](./agents/skills.md)
  + [Crea programma da breve](./agents/program-from-brief.md)
  + [Creare tipi di pubblico per i programmi](./agents/audience-creation.md)
  + [Personalizzare il contenuto delle e-mail per persona](./agents/personalize-content.md)
  + [Crea modelli di punteggio personalizzati](./agents/lead-scoring-model.md)
  + [Creare e promuovere webinar](./agents/webinar-creation.md)
  + [Monitorare ed eseguire il debug della progressione del percorso](./agents/journey-observability.md)
  + [Configurare e analizzare le finalità](./agents/intent.md)
  + [Generare rapporti di analisi](./agents/surface-analytics.md)
+ Decisioning IA {#ai-decisioning}
  + [Panoramica](./agents/ai-decisioning.md)
  + [Integrità delle decisioni basate sull’intelligenza artificiale](./agents/ai-decisioning-health.md)
+ Gestione marketing {#marketing-management}
  + [Panoramica](./marketing/marketing-management.md)
  + Programmi {#programs}
    + [Panoramica](./marketing/programs.md)
    + [Token personalizzati per la personalizzazione](./marketing/personalization-my-tokens.md)
  + [Percorsi di persone](./marketing/person-journeys.md)
  + Area di lavoro e nodi del percorso {#journey-nodes}
    + [Panoramica dei nodi](./marketing/person-journey-nodes.md)
    + [Pubblico persona](./marketing/person-audience-node.md)
    + [Intraprendere un’azione](./marketing/action-nodes.md)
    + [Ascoltare un evento](./marketing/listen-for-event-nodes.md)
    + [Attendere](./marketing/wait-nodes.md)
    + [Dividere e unire i percorsi](./marketing/split-merge-paths-nodes.md)
    + [Prossimo percorso migliore](./marketing/next-best-path.md)
  + Canale e-mail {#email-channel}
    + [Panoramica](./marketing/email-channel.md)
    + [Ottimizzazione dell’ora di invio delle e-mail](./marketing/email-send-time-optimization.md)
    + [Authoring dei contenuti](./content/email-authoring.md)
    + [Progettazione modalità scura](./content/email-dark-mode.md)
  + [Canale WhatsApp](./content/whatsapp-authoring.md)
  + Canale webinar {#webinar-channel}
    + [Panoramica](./marketing/webinars-overview.md)
    + [Creare un webinar](./marketing/create-webinar.md)
    + [Percorsi di webinar](./marketing/webinar-journeys.md)
+ [Controllo traffico percorso](./marketing/journey-traffic-control.md)
+ Rapporti {#reports}
  + [Panoramica](./reports/reports-overview.md)
  + [Panoramica del percorso per persone](./reports/person-journey-overview-report.md)
  + [Singolo Percorso di persone](./reports/person-journey-individual-report.md)
  + [Coinvolgimento](./reports/engagement-report.md)
  + [Coinvolgimento e-mail](./reports/email-engagement-report.md)
  + [Coinvolgimento web](./reports/web-engagement-report.md)
  + [Set di dati di Experience Platform](./reports/aep-datasets.md)
+ Tipi di pubblico {#audiences}
  + [Elenchi di persone](./audiences/people-lists.md)
  + [Dettagli delle persone](./audiences/person-details.md)
  + [Pubblico basato su eventi](./audiences/event-based-audiences.md)
  + [Punteggi di coinvolgimento della persona](./audiences/engagement-scores.md)
  + [Configurazione del modello intento](./audiences/intent-configuration.md)
  + [Utenti tipo derivati](./audiences/personas.md)
  + [Destinazioni](./audiences/destinations.md)
+ Contenuti {#content}
  + Risorse {#assets}
    + [Panoramica](./content/digital-asset-management.md)
    + [Metadati C2PA](./content/c2pa-metadata.md)
  + Modelli {#templates}
    + [Panoramica](./content/templates.md)
    + [Creare modelli](./content/templates-create.md)
    + [Governance dei contenuti](./content/template-content-governance.md)
  + Frammenti {#visual-fragments}
    + [Panoramica](./content/fragments.md)
    + [Authoring dei frammenti](./content/fragment-authoring.md)
  + Pagine di destinazione {#landing-pages}
    + [Panoramica](./content/landing-pages.md)
    + [Creare e pubblicare pagine di destinazione](./content/landing-pages-create-publish.md)
    + [Progettazione pagina di destinazione](./content/landing-page-design.md)
  + Moduli {#forms}
    + [Panoramica](./content/forms.md)
    + [Progettazione del modulo](./content/form-design.md)
  + [Personalizzazione](./content/personalization-expressions.md)
  + [Contenuto condizionale](./content/conditional-content.md)
  + Strumenti di progettazione dei contenuti {#content-design}
    + [Componenti della struttura](./content/structure-components.md)
    + [Componenti per contenuti](./content/content-components.md)
    + [CSS personalizzato](./content/design-custom-css.md)
+ [Scoring Studio](./labs/scoring-studio.md)
+ [Gestione della privacy](./admin/privacy-management.md)
+ Amministrazione {#admin}
  + Canali {#channels}
    + [Configurazione del canale e-mail](./admin/email-channel-configuration.md)
    + [Configurazione del canale WhatsApp](./admin/configuration-channels-whatsapp.md)
    + [Configurazione della pagina di destinazione](./admin/configuration-presets-landing-pages.md)
    + [Configurazione predefinito per modulo](./admin/configuration-presets-forms.md)
  + [Regole di business](./admin/business-rules.md)
  + [Tipi di programmi](./admin/program-types.md)

<!--
   + [Persona mapping](./admin/persona-mapping.md)
-->
